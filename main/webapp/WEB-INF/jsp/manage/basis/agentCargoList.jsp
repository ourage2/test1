...
jQuery(function($) {
	...
	grid1$ = $('#grid1').jqGrid({
		url: _contextPath_ + '/manage/basis/agentCargoList.json',
// 		multiselect: true,
		pager:'#grid1Pg',
		height: 280,
		...
		colNames: [
			'순번','센터명', '대리점명', '대리점코드', '창고명', '단말ID', '단말명', '담당자', '전화번호', '핸드폰', '채널수', 'CH1온도', 'CH2온도', 'CH3온도', 'CH4온도', 'CH5온도', 'CH6온도', '알림여부', '사용여부',
		...
		colModel: [
			...
			{name:'centerNm', index:'CENTER_NM', width: 100},
			{name:'agentNm', index:'AGENT_CD', width: 130, sortable:false},
			{name:'agentCd', index:'AGENT_CD', width: 100},
			{name:'cargoNm', index:'CARGO_NM', width: 110},
			{name:'devId', index:'DEV_ID', width: 50, key: true},
			{name:'devNm', index:'DEV_NM', width: 100},
			{name:'chargeNm', index:'CHARGE_ID', width: 80, sortable:false},
			{name:'chargeTel', index:'CHARGE_ID', width: 90, sortable:false, formatter: 'tel'},
			{name:'chargeHp', index:'CHARGE_ID', width: 90, sortable:false, formatter: 'tel'},
			...
			{name:'ch5Dp', index:'CH5_DP', width: 60, align:'center', sortable:false},
			{name:'ch6Dp', index:'CH6_DP', width: 60, align:'center', sortable:false},
			{name:'pushYnNm', index:'PUSH_YN', width: 60},
...
// 	$('#btnGrid1Del').on('click', function() {user.cargoDevBatchSave('D');}); //일괄삭제
	...
	$('#btnGrid1Xls').on('click', function() {
		cvo.ajaxFileDown({url: _contextPath_ + '/manage/basis/agentCargoList.xls', data: com.getData(srch$)});
	});
...
// 		com.setCombo('select', view1$, 'centerCd', null, gCenterList);
	...
	cargoDevSave: function(action) {
		...
		onm.ajax({
			url: _contextPath_ + '/manage/basis/agentCargoSave.json',
			beforeSend: function() {
...
// 	},
	...
	userFindPop: function() {
		onm.ajaxModal({
			...
			data: {srchAgentCd: view1$.find('[name=agentCd]').val()},
...
<header>
	<div id="divSrch">
		<div class="loc_info"><span>대리점창고관리</span><span>기준정보</span><span>Home</span></div>
		<h2 class="content_title"><span>대리점창고관리</span></h2>
		<div class="srch_box">
			<table>
				<colgroup>
					<col width="7%">
				...
				<tr>
					...
					<th>대리점명</th>
					<td>
						<input type="text" name="srchAgentNm"  maxlength="20" class="enterSrch" />
					</td>
					<th>창고명</th>
					<td>
			...
			<table class="dtl_tbl">
				...
				<tbody>
					<tr>
						...
						<td><input type="text" name="devId" maxlength="20" read /></td>
						<th>단말명</th>
						<td><input type="text" name="devNm" maxlength="20" read /></td>
					</tr>
					<tr>
