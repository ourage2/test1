...
jQuery(function($) {
	...
	grid1$ = $('#grid1').jqGrid({
		url: _contextPath_ + '/manage/basis/agentCarList.json',
		pager:'#grid1Pg',
		height: 260,
		...
		colNames:[
			'순번', '센터명', '대리점명', '대리점코드', '차량ID', '차량번호', '단말기번호', '운전자명', '전화번호', '핸드폰', '차종(톤)', '채널수', 'CH1온도', 'CH2온도', 'CH3온도', 'CH4온도', 'DTG연동', '알림여부', '사용여부',
			'centerCd', 'wetCd', 'useYn', 'dtgYn', 'ch1Min', 'ch1Max', 'ch2Min', 'ch2Max', 'ch3Min', 'ch3Max', 'ch4Min', 'ch4Max',
		...
		colModel:[
			...
			{name:'agentNm', index:'AGENT_CD', width: 130, sortable:false},
			{name:'agentCd', index:'AGENT_CD', width: 100},
	...
	$('#btnGrid1Xls').on('click', function() {
		cvo.ajaxFileDown({url: _contextPath_ + '/manage/basis/agentCarList.xls', data: com.getData(srch$)});
	});
...
// 		com.setCombo('select', view1$, 'centerCd', null, gCenterList);
	...
	carInit: function() {
		...
		view1$.find('[name=agentNm]').prop('readonly', true);
		view1$.find('[name=useYn][value=Y]').prop('checked', true);
		view1$.find('[name=dtgYn][value=Y]').prop('checked', true);
	...
	carSave: function(action) {
		...
		onm.ajax({
			url: _contextPath_ + '/manage/basis/agentCarSave.json',
			beforeSend: function() {
	...
	agentFindPop: function() {
		onm.ajaxModal({
			...
			callback:function(row) {
				view1$.put('centerNm', row.centerNm);
	...
	pushFindPop: function() {
		onm.ajaxModal({
			...
			data: {
				...
				srchAcDiv : 'A', //대리점 사용자만 검색
				srchAgentCd: view1$.find('[name=agentCd]').val(),
	...
	bizValid: function(area) {
		if (area.attr('id') == 'divView1') {
			if (area.find('[name=carNo]').val()) {
				if (!fnValidCarNo(area.find('[name=carNo]').val())) {
					onm.alert('차량번호가 올바른 형식이 아닙니다.');
					return false;
				}
			}
			return true;
...
<header>
	<div id="divSrch">
		<div class="loc_info"><span>대리점차량관리</span><span>기준정보</span><span>Home</span></div>
		<h2 class="content_title"><span>대리점차량관리</span></h2>
		<div class="srch_box">
			<table>
				<colgroup>
					<col width="10%">
				...
				<tr>
					...
					<td colspan="4">
						<select name="srchMethod">
							...
							<option value="agentNm">대리점명</option>
		...
		<div id="divView1">
			...
			<table class="dtl_tbl">
				...
				<tbody>
					<tr>
						<th>소속센터</th>
						<td><input type="text" name="centerNm" maxlength="50" read noinput /></td>
						<th class="state-required"><span class="icon_key" title="key(중복주의)"></span>차량ID</th>
						<td><input type="text" name="carId" maxlength="5" read noinput /></td>
						<th class="state-required">대리점<button type="button" id="btnAgentFind" class="btn_list_sm mT5"><span class="ico_srch"></span>찾기</button></th>
						<td><input type="text" name="agentNm" maxlength="50" read /></td>
						<th>차량번호</th>
						<td><input type="text" name="carNo" maxlength="15" /></td>
						<th>차종(톤)</th>
						<td><select name="wetCd"></select></td>
					</tr>
					<tr>
						<th>운전자명</th>
						<td><input type="text" name="drvNm" maxlength="20" /></td>
						<th>전화번호</th>
						<td><input type="text" name="drvTelNo" maxlength="13" tel /></td>
						<th>핸드폰</th>
						<td><input type="text" name="drvHpNo" maxlength="13" tel /></td>
