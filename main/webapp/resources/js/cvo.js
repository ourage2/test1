...
//		},
		...
		download: function(options) {
			...
			if(settings.downloadTarget) {
				var ajaxData = cvo.ajaxData(settings.downloadTarget);
				if(ajaxData) {
...
//	    	console.log(settings.url);
	    	var uri = location.href;
	    	uri = uri.substring(uri.lastIndexOf("/") + 1, uri.lastIndexOf("."));
	    	settings.url = settings.url + '?menuCd=' + uri;
//	    	if (settings.url.indexOf('xls') > -1 || settings.url.indexOf('xlsx') > -1 ) {
//	    		settings.headers = {'X-Accept': 'application/vnd.ms-excel'};
//	    	}
//	    	console.log(settings);
		...
		handleError: function(response) {
			if(response) {
				if(cvo._type(response) === 'string') {
					response = $.parseJSON(response);
				}
				if(response.resCd === _unauthorizedCode_) {
//					onm.alert('세션이 종료되었습니다.\n로그인 페이지로 이동합니다.');
					...
					onm.alert('[' + response.resCd + '] ' + response.resMsg);
				}
			}
		},
		_getAjaxOptions: function(options) {
			...
			if(cvo._type(responseTarget$) !== 'jquery') {
				...
				for(var i=0, s=newParamToken.length; i<s; i++) {
					...
					if(matcher && matcher.length === 3) {
						if(matcher[2]) {
							...
							if(cvo._type(json[matcher[1]]) === 'string') {
								json[matcher[1]] = new Array(json[matcher[1]]);
								json[matcher[1]].push(value);
							} else if(cvo._type(json[matcher[1]]) === 'array') {
								json[matcher[1]].push(value);
							} else {
...
$.fn.extend({
	...
	serializeJson: function() {
		 ...
		 $.each(arr, function(i, v) {
			 if(v.value) {
				 if(cvo._type(json[v.name]) === 'string') {
					 json[v.name] = new Array(json[v.name]);
					 json[v.name].push(v.value);
				 } else if(cvo._type(json[v.name]) === 'array') {
					 json[v.name].push(v.value);
				 } else {
					 json[v.name] = v.value;
				 }
...
$.extend(true, window, {
	cvo: {
		...
		ajaxDown: function(options) {
			AjaxUtil.download(options);
		},
		ajaxFileDown: function(options) {
...
jQuery(function($) {
	...
	if(modalLoader$.length !== 1) {
		modalLoader$ = $('<div id="modal-loader" class="modal" style="display:none;" role="dialog"><div class="modal-dialog"><img src="' + _contextPath_ + '/resources/images/common//ajax-loading.gif" /></div></div>').appendTo('body');
	}
	cvo._setAjaxLoading(modalLoader$);
	cvo._objectInitialize();
});
