...
jq.widget( "ui.pattern", jq.ui.mouse, {
	...
	_create: function() {
		...
		linecontainer = jq( "<canvas></canvas>" );
		...
		this._nodecontainer = jq( "<div></div>" )
		.addClass( "ui-patter-node-container" )
		.appendTo( this.element );

		if (this._canvasSupported === true) {
			this._arrowcontainer = jq( "<canvas></canvas>" )
	...
	_mouseDrag: function( event ) {
		...
		.filter( function() {
			return this !== self._previousNode[0] && ( self.options.multiSelect === true || jq(this).attr( "selected" ) !== "selected" );
		} )
		.hitTest( event.pageX - this._nodecontainer.offset().left, event.pageY - this._nodecontainer.offset().top );

		if ( node.length ) {
			this._selectNode( jq(node[0]), event );
		}
	...
	_clear: function() {
		...
		this.element
		.find( ".ui-pattern-node" )
		.filter( function() { return jq(this).attr( "selected" ) === "selected"; } )
	...
	_createGrid: function() {
		...
		for ( y = 0; y < this.options.gridSize; y++ ) {
			row = jq( "<div></div>" ).addClass( "ui-pattern-row" );
	...
	_createNode: function( x, y ) {
		...
		.focus( function() {
			if ( self.options.disabled !== true ) {
				jq( ".ui-pattern-node .ui-state-focus" ).removeClass( "ui-state-focus" );
		...
		.keydown( function( event ) {
			...
			nodeX = parseInt( jq(this).data( "x.ui-pattern-node" ), 10 ),
			nodeY = parseInt( jq(this).data( "y.ui-pattern-node" ), 10 );

			if ( event.keyCode === jq.ui.keyCode.UP ) {
				if ( nodeY > 0 ) {
					node = self.element
					.find( ".ui-pattern-node" )
					.filter(function() {
						return parseInt( jq(this).data( "x.ui-pattern-node" ), 10 ) === nodeX && parseInt( jq(this).data( "y.ui-pattern-node" ), 10 ) === nodeY - 1;
					} );
			...
			} else if ( event.keyCode === jq.ui.keyCode.DOWN ) {
			...
			} else if ( event.keyCode === jq.ui.keyCode.LEFT ) {
				if ( nodeX > 0 ) {
					node = self.element
					.find( ".ui-pattern-node" )
					.filter( function() {
						return parseInt( jq(this).data( "x.ui-pattern-node" ), 10 ) === nodeX - 1 && parseInt( jq(this).data( "y.ui-pattern-node" ), 10 ) === nodeY;
					} );
			...
			} else if ( event.keyCode === jq.ui.keyCode.RIGHT ) {
			...
			} else if ( event.keyCode === jq.ui.keyCode.SPACE ) {
				...
				.find( ".ui-pattern-node" )
				.filter( function() {
					return jq(this).attr( "selected" ) === "selected";
				} );
				...
				if ( ( !self._previousNode || this !== self._previousNode[0] ) && ( self.options.multiSelect === true || jq(this).attr( "selected" ) !== "selected" ) ) {
					self._selectNode( jq(this), event );
			...
			} else if ( event.keyCode === jq.ui.keyCode.ENTER ) {
				if ( self.options.disabled === true || self._waitingForClear === true ) {
					return;
				}
	...
	_drawArrow: function( from, to, correct ) {
		...
		context = this._arrowcontainer[0].getContext( "2d" );
		if ( correct === true) {
			image = jq(this._arrowCorrect);
		} else {
			image = jq(this._arrowIncorrect);
		}
	...
	_getNode: function( x, y, directionX, directionY ) {
		...
		.find( ".ui-pattern-node" )
		.filter( function() {
			return parseInt( jq(this).data( "x.ui-pattern-node" ), 10 ) === x && parseInt( jq(this).data( "y.ui-pattern-node" ), 10 ) === y;
		} );
	...
	_selectNode: function( node, event ) {
		...
		if ( previousNode !== null) {
			...
			} else if ( nodeY === previousNodeY ) {
				if ( nodeX > previousNodeX ) {
					nextNode = this._getNode( previousNodeX, nodeY, 1, 0 );
				} else {
					nextNode = this._getNode( previousNodeX, nodeY, -1, 0 );
				}
			} else if ( nodeY - nodeX === previousNodeY - previousNodeX ) {
				if ( nodeY > previousNodeY ) {
	...
	_setCanvasWidth: function() {
		...
		if ( nodes.length ) {
			width = jq(nodes[0]).outerWidth( true ) * this.options.gridSize;
			height = jq(nodes[0]).outerHeight( true ) * this.options.gridSize;
		}
...
jq.extend( jq.ui.pattern, {
		version: "@VERSION"
} );

jq.fn.hitTest = function( x, y ) {
	var matched = [];

	this.each( function() {
		var position = jq(this).position(),
		marginLeft = parseInt( jq(this).css( "margin-left" ), 10 ) || 0,
		marginTop = parseInt( jq(this).css( "margin-top" ), 10 ) || 0;

		if ( position.left + marginLeft <= x && position.left + marginLeft + jq(this).outerWidth() >= x && position.top + marginTop <= y && position.top + marginTop + jq(this).outerHeight() >= y ) {
			matched.push( this );
		}
	} );
